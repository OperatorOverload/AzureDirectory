...
namespace Lucene.Net.Store.Azure
{
    ...
    public class AzureLock : Lock
    {
        ...
        public override bool Obtain()
        {
            ...
            try
            {
                Trace.TraceInformation("AzureLock:Obtain({0}) : {1}", _lockFile, _leaseid);
                if (String.IsNullOrEmpty(_leaseid))
                {
                    _leaseid = blob.AcquireLease(TimeSpan.FromSeconds(60), _leaseid);
                    Trace.TraceInformation("AzureLock:Obtain({0}): AcquireLease : {1}", _lockFile, _leaseid);
                    ...
                    _renewTimer = new Timer((obj) => 
                        {
                            ...
                            catch (Exception err) { Trace.TraceInformation(err.ToString()); } 
        ...
        public void Renew()
        {
            if (!String.IsNullOrEmpty(_leaseid))
            {
                Trace.TraceInformation("AzureLock:Renew({0} : {1}", _lockFile, _leaseid);
                var blob = _azureDirectory.BlobContainer.GetBlockBlobReference(_lockFile);
        ...
        {
            Trace.TraceInformation("AzureLock:Release({0}) {1}", _lockFile, _leaseid);
            if (!String.IsNullOrEmpty(_leaseid))
            {
                var blob = _azureDirectory.BlobContainer.GetBlockBlobReference(_lockFile);
                blob.ReleaseLease(new AccessCondition { LeaseId = _leaseid });
        ...
        {
            Trace.TraceInformation("AzureLock:BreakLock({0}) {1}", _lockFile, _leaseid);
            var blob = _azureDirectory.BlobContainer.GetBlockBlobReference(_lockFile);
            try
            {
                blob.BreakLease();
